...
Dygraph.prototype.__init__ = function(a, c, l) {
	...
	if (!a) {
		Dygraph.error('Constructing dygraph with a non-existent div!: ', a);
		return;
	}
...
/** step5 **/
Dygraph.prototype.extractSeries_ = function(rawData, serieIndex, isLogScale) {
	var g = [];
	for (var d = 0; d < rawData.length; d++) {
		var l = rawData[d][0];
		var m = rawData[d][serieIndex];
		g.push([l, m]);		
	}
	return g;
};
...
/** step 6 
	TODO: récupérer toutes les données des séries, virer les commentaires, etc...
**/
