...
DygraphLayout.prototype._evaluateLineCharts=function(){
	var c=this.attr_("connectSeparatedPoints");var k=this.attr_("stackedGraph");
	...
	for(var a=0;a<this.points.length;a++){
		...
		for(var d=0;d<l.length;d++){
			...
			var h=i.yval;if(k){i.y_stacked=DygraphLayout._calcYNormal(b,i.yval_stacked,g);if(h!==null&&!isNaN(h)){h=i.yval_stacked}}
			if(h===null){
				h=NaN;
				if(!c){
					i.yval=NaN
...
DygraphCanvasRenderer._Plotters={
	...
	candlePlotter:function(a){					//modif aep: ajout 3 lignes
		DygraphCanvasRenderer._candlePlotter(a)
	}
};
DygraphCanvasRenderer._candlePlotter = function(e) { //modif aep: ajout de cette fonction
	...
	if (setCount != 4) {
		throw "Exactly 4 columns each point must be provided for candle chart (low close open high), you provided " + setCount + " columns";
	}
	var prices = [];
	var price;
...
DygraphCanvasRenderer._errorPlotter=function(s){
	...
	var p=s.points;
	var q = Dygraph.createIterator(p, 0, p.length, DygraphCanvasRenderer._getIteratorPredicate(r.getOption("connectSeparatedPoints")));	//toto
	var j;
	var h=NaN;var c=NaN;var d=[-1,-1];
	var a=new RGBColorParser(n);
	//var u="rgba("+a.r+","+a.g+","+a.b+","+o+")";		//modif aep
	var u="rgba("+a.r+","+a.g+","+a.b+", 0.2)";		//modif aep
	//var u= 'rgba(0, 255, 0, 0.2)';		//modif aep
	m.fillStyle=u;
	m.beginPath();
	...
	while(q.hasNext){
		...
		if((!i&&b(l.y))||(i&&!isNaN(c)&&b(c))){
			//h=NaN;
			//continue;		//modif aep pour enveloppe
			//h = l.canvasx;
		}
		if(i){
...
Dygraph.prototype.extremeValues_=function(d){
	...
	if(b){
		for(c=0;c<d.length;c++){
			...
			var a=g-d[c][1][1];
			var e=g+d[c][1][2];if(a>g){a=g}if(e<g){e=g}if(f===null||e>f){f=e}if(h===null||a<h){h=a}
		}
	}
	else{
		for(c=0;c<d.length;c++){g=d[c][1];if(g===null||isNaN(g)){continue}if(f===null||g>f){f=g}if(h===null||g<h){h=g}}
	}
	return[h,f]
};
	
Dygraph.prototype.predraw_=function(){
		var e=new Date();this.layout_.computePlotArea();this.computeYAxes_();
		if(this.plotter_){
		...
		this.canvas_ctx_.save();this.hidden_ctx_.save();
...
Dygraph.prototype.seriesToPoints_=function(serieData,j,serieName,f){

	//toto
	console.log('##getPropertiesForSeries(serieName)', this.getPropertiesForSeries(serieName), this.getOption('series'));
	var h=[];
	for(var c=0; c<serieData.length; ++c){
		var k=serieData[c];
		var a=j?k[1][0]:k[1];
...
Dygraph.stackPoints_=function(m,l,o,f){
	var h=null;var d=null;
	var g=null;var c=-1;var k=function(i){if(c>=i){return}for(var p=i;p<m.length;++p){g=null;if(!isNaN(m[p].yval)&&m[p].yval!==null){c=p;g=m[p];break}}};
	for(var b=0;b<m.length;++b){var j=m[b];var n=j.xval;if(l[n]===undefined){l[n]=0}var e=j.yval;if(isNaN(e)||e===null){k(b);
	if(d&&g&&f!="none"){e=d.yval+(g.yval-d.yval)*((n-d.xval)/(g.xval-d.xval))}else{if(d&&f=="all"){e=d.yval}else{if(g&&f=="all"){e=g.yval}else{e=0}}}}
	else{d=j}var a=l[n];if(h!=n){a+=e;l[n]=a}h=n;j.yval_stacked=a;
	if(a>o[1]){o[1]=a}if(a<o[0]){o[0]=a}}
};
	...
	for(u=n;u>=1;u--){
		...
		if(f){
			...
			for(r=0;r<l.length;r++){
				if(l[r][0]>=z&&g===null){g=r}if(l[r][0]<=j){y=r}
			}
			if(g===null){g=0}var e=g;var d=true;while(d&&e>0){e--;d=b(l[e])}if(y===null){y=l.length-1}var c=y;d=true;while(d&&c<l.length-1){c++;d=b(l[c])}if(e!==g){g=e}if(c!==y){y=c}s[u-1]=[g,y];l=l.slice(g,y+1)		
		}
		else{
			l=w[u];
			s[u-1]=[0,l.length-1];
		...
		var m = this.seriesToPoints_(l, p,v,s[u-1][0]);	//modif aep: passage de static à non-static
...
Dygraph.prototype.drawGraph_=function(){
	...
	for(var f=1;f<h.length;f++){
		...
		if(!this.visibility()[f-1]){continue}
	...
	if(this.fractions_){
		...
		if(this.attr_("customBars")){
			...
			for(z=0;z<l.length;z++){
				...
				if(z-d>=0){
					var r=l[z-d];if(r[1][1]!==null&&!isNaN(r[1][1])){G-=r[1][0];D-=r[1][1];o-=r[1][2];g-=1}
				}
				if(g){
					b[z]=[l[z][0],[1*D/g,1*(D-G)/g,1*(o-D)/g]]
				}
				/*else{									//modif aep: mise en commentaire pour enveloppe
			...
			if(!this.attr_("errorBars")){
				if(d==1){return l}for(z=0;z<l.length;z++){c=0;F=0;for(x=Math.max(0,z-d+1);x<z+1;x++){m=l[x][1];if(m===null||isNaN(m)){continue}F++;c+=l[x][1]}if(F){b[z]=[l[z][0],c/F]}else{b[z]=[l[z][0],null]}}
			}
			else{
				for(z=0;z<l.length;z++){c=0;var f=0;F=0;for(x=Math.max(0,z-d+1);x<z+1;x++){m=l[x][1][0];if(m===null||isNaN(m)){continue}F++;c+=l[x][1][0];f+=Math.pow(l[x][1][1],2)}if(F){A=Math.sqrt(f)/F;b[z]=[l[z][0],[c/F,t*A,t*A]]}else{var q=(d==1)?l[z][1][0]:null;b[z]=[l[z][0],[q,q,q]]}}
			}
		}
	}
	return b;
		...
		if(this.attr_("customBars")){
			for(k=1;k<d.length;k++){
				...
				if(/^ *$/.test(u)){
					...
					g=u.split(";");if(g.length==3){h[k]=[this.parseFloat_(g[0],l,e),this.parseFloat_(g[1],l,e),this.parseFloat_(g[2],l,e)]}
...
Dygraph.Plugins.Legend=(function(){
	...
	a=function(w,p,l,f){
		...
		var isFullCustomLegendX = A('isFullCustomLegend');	//modif aep
		...
		if(typeof(p)==="undefined"){
			...
			if(nonEmptyLengendOnStartUp){		//do the same thing as if a point was selected -- !TODO: factorize this code!
				r = o(null, A, z[0], w);	//add date legend
			}
			for(u=1;u<z.length;u++){
				...
				if(r!=="" && isFullCustomLegendX){
					r+=(C?"<br/>":" ")
				}
				m = w.getOption("strokePattern",z[u]);
				s = d(m,q.color,f);
				//r+="<span style='font-weight: bold; color: "+q.color+";'>"+s+" "+z[u]+"</span>";	//code initial
				if(nonEmptyLengendOnStartUp){
					var axisOptions = v[q.axis-1];
					var valueFormatterFunction = axisOptions("valueFormatter");
					var e = valueFormatterFunction(null, n, z[u], w);
					r+= e;	//add value legend
				}
				else{
					r+="<span style='font-weight: bold; color:#000000;'>"+s+" "+z[u]+"</span>";	//modif aep
				}
			}
			return r;
		}
		...
		for(u=0;u<l.length;u++){
			...
			var isFullCustomLegend = n('isFullCustomLegend');
...
d=function(s,h,r){
	...
	for(l=0;l<=s.length;l++){g+=s[l%s.length]}n=Math.floor(r/(g-s[0]));if(n>1){for(l=0;l<s.length;l++){p[l]=s[l]/r}q=p.length}else{n=1;for(l=0;l<s.length;l++){p[l]=s[l]/g}q=p.length+1}var m="";
for(k=0;k<n;k++){for(l=0;l<q;l+=2){f=p[l%p.length];if(l<s.length){o=p[(l+1)%p.length]}else{o=0}m+='<div style="display: inline-block; position: relative; bottom: .5ex; margin-right: '+o+"em; padding-left: "+f+"em; height: 1px; border-bottom: 2px solid "+h+';"></div>'}}return m};return c}
...
Dygraph.Export.yTranslation = 70;		//modif aep: translation to the bottom to make room for the title
