...
DygraphLayout.prototype._evaluateLineCharts=function(){
	...
	for(var a=0;a<this.points.length;a++){
		...
		for(var d=0;d<l.length;d++){
			...
			var h=i.yval;
			if(k){i.y_stacked=DygraphLayout._calcYNormal(b,i.yval_stacked,g);if(h!==null&&!isNaN(h)){h=i.yval_stacked}}if(h===null){h=NaN;if(!c){i.yval=NaN}}i.y=DygraphLayout._calcYNormal(b,h,g);
			if(e){
...
Dygraph.prototype.predraw_=function(){var e=new Date();this.layout_.computePlotArea();this.computeYAxes_();if(this.plotter_){this.cascadeEvents_("clearChart");this.plotter_.clear()}if(!this.is_initial_draw_){this.canvas_ctx_.restore();this.hidden_ctx_.restore()}this.canvas_ctx_.save();this.hidden_ctx_.save();this.plotter_=new DygraphCanvasRenderer(this,this.hidden_,this.hidden_ctx_,this.layout_);this.createRollInterface_();this.cascadeEvents_("predraw");this.rolledSeries_=[null];for(var c=1;c<this.numColumns();c++){var d=this.attr_("logscale");var b=this.extractSeries_(this.rawData_,c,d);b=this.rollingAverage(b,this.rollPeriod_);this.rolledSeries_.push(b)}this.drawGraph_();var a=new Date();this.drawingTimeMs_=(a-e)};


Dygraph.PointType=undefined;

Dygraph.seriesToPoints_=function(b,j,d,f){
	var h=[];for(var c=0;c<b.length;++c){var k=b[c];var a=j?k[1][0]:k[1];var e=a===null?null:DygraphLayout.parseFloat_(a);var g={x:NaN,y:NaN,xval:DygraphLayout.parseFloat_(k[0]),yval:e,name:d,idx:c+f};if(j){g.y_top=NaN;g.y_bottom=NaN;g.yval_minus=DygraphLayout.parseFloat_(k[1][1]);g.yval_plus=DygraphLayout.parseFloat_(k[1][2])}h.push(g)}return h
	};
...
Dygraph.Plugins.Legend=(function(){
	...
	c.prototype.select=function(i){		//mouseover on the chart
		...
		var f = createLegendFunction(i.dygraph, h, g, this.one_em_width_);
		this.legend_div_.innerHTML = '';
		this.legend_div_.append(f);
	};
    ...
    c.prototype.deselect=function(h){	//mouseout on the chart
		...
		this.legend_div_.innerHTML = '';
		this.legend_div_.append(g);
	};
	
	c.prototype.didDrawChart=function(f){this.deselect(f)};
	c.prototype.predraw=function(h){
		if(!this.is_generated_div_){return}
		h.dygraph.graphDiv.appendChild(this.legend_div_);var g=h.dygraph.plotter_.area;var f=h.dygraph.getOption("labelsDivWidth");
	...
	createLegendFunction = function(w,p,l,f){
		...
		if(typeof(p)==="undefined"){	//called when the mouse goes out of the chart			
			...
			if(nonEmptyLengendOnStartUp){		//do the same thing as if a point was selected -- !TODO: factorize this code!
				result.append(valueFormatterFunction(null, A, z[0], w));	//add date legend
			}
			for(u=1;u<z.length;u++){
				...
				result.append(e);	//add value legend
			}
			
			return result;
		}
		
		var result = valueFormatterFunction(p,A,z[0],w);
		...
		for(u=0;u<l.length;u++){
			...
			var n=v[q.axis-1];
			var valueFormatterFunction = n("valueFormatter");
			var e = valueFormatterFunction(t.yval, n, t.name, w);
